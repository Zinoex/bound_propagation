...
class TestElementwiseLinearRelaxationForwardCompose:
    ...
    def test_basic_positive_alpha(self):
        ...
        relaxation = ElementwiseLinearRelaxation(
    ...
    def test_identity_relaxation_passthrough(self):
        ...
        input_bounds = LinearBounds(region, wl, bl, wu, bu)
    ...
    def test_wrong_number_of_inputs_raises(self):
        """Passing != 1 input raises ValueError."""
        relaxation = ElementwiseLinearRelaxation(
            alpha_lower=torch.tensor([1.0]),
            beta_lower=torch.tensor([0.0]),
            alpha_upper=torch.tensor([1.0]),
        ...
        region = HyperRectangle(torch.tensor([0.0]), torch.tensor([1.0]))
        bounds = LinearBounds(
            region, torch.tensor([[1.0]]), torch.tensor([0.0]), torch.tensor([[2.0]]), torch.tensor([1.0])
        )
...
class TestPairedLinearRelaxationForwardCompose:
    ...
    def test_basic_positive_coeffs_distinct_input_ids(self):
        ...
        # coeffs_lower ≤ coeffs_upper so the composed bounds are ordered lower ≤ upper
        relaxation = PairedLinearRelaxation(
            coeffs_lower=[torch.tensor([1.0]), torch.tensor([1.0])],
            coeffs_upper=[torch.tensor([2.0]), torch.tensor([3.0])],
            bias_lower=torch.tensor([0.0]),
    ...
    def test_negative_coefficients_swap_bounds(self):
        ...
        relaxation = PairedLinearRelaxation(
            coeffs_lower=[torch.tensor([1.0]), torch.tensor([-2.0])],
            coeffs_upper=[torch.tensor([1.0]), torch.tensor([-1.0])],
            bias_lower=torch.tensor([0.0]),
    ...
    def test_both_inputs_constant(self):
        ...
        relaxation = PairedLinearRelaxation(
            coeffs_lower=[torch.tensor([1.0]), torch.tensor([2.0])],
            coeffs_upper=[torch.tensor([1.0]), torch.tensor([3.0])],
            bias_lower=torch.tensor([0.5]),
            bias_upper=torch.tensor([1.0]),
        )
    ...
    def test_one_input_constant(self):
        ...
        relaxation = PairedLinearRelaxation(
            coeffs_lower=[torch.tensor([2.0]), torch.tensor([1.0])],
            coeffs_upper=[torch.tensor([3.0]), torch.tensor([1.0])],
            bias_lower=torch.tensor([0.0]),
            bias_upper=torch.tensor([0.0]),
        )
    ...
    def test_multidimensional_inputs(self):
        ...
        relaxation = PairedLinearRelaxation(
            coeffs_lower=[torch.tensor([1.0, 1.0]), torch.tensor([1.0, 1.0])],
            coeffs_upper=[torch.tensor([2.0, 2.0]), torch.tensor([3.0, 3.0])],
            bias_lower=torch.tensor([0.0, 0.0]),
    ...
    def test_1d_batch_distinct_input_ids(self):
        ...
        # coeffs shape: (B=2, D=1); coeffs_lower ≤ coeffs_upper
        relaxation = PairedLinearRelaxation(
            coeffs_lower=[torch.ones(2, 1), torch.ones(2, 1)],
            coeffs_upper=[2 * torch.ones(2, 1), 2 * torch.ones(2, 1)],
            bias_lower=torch.zeros(2, 1),
    ...
    def test_2d_batch_shared_input_id(self):
        ...
        relaxation = PairedLinearRelaxation(
            coeffs_lower=[torch.ones(2, 2, 1), torch.ones(2, 2, 1)],
            coeffs_upper=[2 * torch.ones(2, 2, 1), torch.ones(2, 2, 1)],
            bias_lower=torch.zeros(2, 2, 1),
    ...
    def test_wrong_number_of_inputs_raises(self):
        """Passing != 2 inputs raises ValueError."""
        relaxation = PairedLinearRelaxation(
            coeffs_lower=[torch.tensor([1.0]), torch.tensor([1.0])],
            coeffs_upper=[torch.tensor([1.0]), torch.tensor([1.0])],
            bias_lower=torch.tensor([0.0]),
    ...
    def test_invalid_construction_non_binary(self):
        ...
        with pytest.raises(ValueError, match="exactly 2"):
            PairedLinearRelaxation(
                coeffs_lower=[torch.tensor([1.0])],
                coeffs_upper=[torch.tensor([1.0])],
                bias_lower=torch.tensor([0.0]),
                bias_upper=torch.tensor([0.0]),
            )

    def test_invalid_construction_mismatched_coeff_lengths(self):
        """coeffs_lower and coeffs_upper must have the same length."""
