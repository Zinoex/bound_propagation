...
def test_exp_composition_with_log() -> None:
    ...
    # log(a)
    log_a = log_strategy.propagate_forwards(None, [a])  # ty:ignore[invalid-argument-type]

    # exp(log(a))
    result = exp_strategy.propagate_forwards(None, [log_a])  # ty:ignore[invalid-argument-type]
