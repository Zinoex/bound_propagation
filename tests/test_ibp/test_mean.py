...
from bound_propagation.propagation.ibp.mean import IBPMean
...
def _propagate(lower: torch.Tensor, upper: torch.Tensor, dim: int = 0) -> IntervalBounds:
    ...
    # Create a simple node with mean operation
    ...
    node = Node(id=0, op_type=OperationType.MEAN, inputs=[], output_metadata=metadata, attributes={"dim": dim})

    return strategy.propagate_forwards(node, input_bounds=[bounds])


def _compute_output_shape(input_shape: tuple[int, ...], dim: int) -> tuple[int, ...]:
    """Compute output shape after mean reduction."""
...
def test_mean_monotonicity() -> None:
    ...
    node = Node(id=0, op_type=OperationType.MEAN, inputs=[], output_metadata=metadata, attributes={"dim": 1})
...
def test_mean_relation_to_sum() -> None:
    ...
    sum_metadata = TensorMetadata(shape=(1,), dtype=lower.dtype, device=lower.device)
    sum_node = Node(
        id=0,
        op_type=OperationType.SUM,
        inputs=[],
        output_metadata=sum_metadata,
        attributes={"dim": 1, "keepdim": False},
    )
    sum_out = sum_strategy.propagate_forwards(sum_node, [bounds])
