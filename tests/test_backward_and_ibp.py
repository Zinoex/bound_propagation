...
class TestBackwardLBPPropagator:
    ...
    def test_relaxation_strategies_are_reused_during_propagation(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "bound_propagation.propagation.relaxations.base.RelaxationRegistry.get",
            lambda op_type: (_ for _ in ()).throw(AssertionError("registry lookup during propagate")),
        )
        monkeypatch.setattr(
            "bound_propagation.propagation.relaxations.base.RelaxationRegistry.has_strategy",
            lambda op_type: (_ for _ in ()).throw(AssertionError("registry lookup during propagate")),
        )
    ...
    def test_simple_linear_backward(self):
        ...
        W = torch.tensor([[1.0, 2.0, 0.5], [0.5, 1.0, 2.0]])
    ...
    def test_backward_through_add(self):
        ...
        output_bounds = {add_node.id: IntervalBounds(lower=output_lower, upper=output_upper)}
