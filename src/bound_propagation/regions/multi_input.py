...
class MultiInputRegion(AbstractRegion):
    ...
    def lower(self) -> torch.Tensor:
        ...
        For single-input compatibility, returns the first region's lower bound.
        """
        return next(iter(self.regions.values())).lower
    ...
    def upper(self) -> torch.Tensor:
        ...
        For single-input compatibility, returns the first region's upper bound.
        """
        return next(iter(self.regions.values())).upper
    ...
    def __getitem__(self, input_id: int) -> HyperRectangle:
        """Get region for a specific input node."""
        if input_id not in self.regions:
            raise KeyError(f"No region found for input node {input_id}")
        return self.regions[input_id]
    
    def get(self, input_id: int, default: HyperRectangle | None = None) -> HyperRectangle | None:
        """
        ...
        """
        return self.regions.get(input_id, default)
    ...
    def maximize(self, direction: torch.Tensor) -> torch.Tensor:
        ...
        """
        return next(iter(self.regions.values())).maximize(direction)
    ...
    def minimize(self, direction: torch.Tensor) -> torch.Tensor:
        ...
        """
        return next(iter(self.regions.values())).minimize(direction)
