...
class ReluRelaxationStrategy(RelaxationStrategy):
    ...
    def relax(
        ...
        if len(interval_inputs) != 1:
            raise ValueError(f"ReLU expects 1 input, got {len(interval_inputs)}")

        input_bounds = interval_inputs[0]
        lower, upper = input_bounds.concretize()

        # Compute alpha/beta parameters
        alpha_lower, beta_lower, alpha_upper, beta_upper = compute_relu_alpha_beta(lower, upper, adaptive=self.adaptive)
